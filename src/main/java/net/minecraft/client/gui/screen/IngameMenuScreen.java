...
public class IngameMenuScreen extends Screen
{
    ...
    private void addButtons()
    {
        ...
        Button button1 = this.addButton(new Button(this.width / 2 - 102, this.height / 4 + 120 + -16, 204, 20, new TranslationTextComponent("menu.returnToMenu"), (button2) ->
        {
            ...
            if (flag)
            {
                this.minecraft.displayGuiScreen(new NeironMainMenu());
            }
            else if (flag1)
            {
                RealmsBridgeScreen realmsbridgescreen = new RealmsBridgeScreen();
                realmsbridgescreen.func_231394_a_(new NeironMainMenu());
            }
            else {
                this.minecraft.displayGuiScreen(new MultiplayerScreen(new NeironMainMenu()));
            }
        }));

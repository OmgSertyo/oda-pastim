...
public class AutoBuyButton {
    ...
    public sertyo.events.ui.ab.manager.AutoBuyItem autoBuyItem;
    public String name;
    ...
    public AutoBuyButton(double x, double y, double w, double h, sertyo.events.ui.ab.manager.AutoBuyItem autoBuyItem, String name) {
        this.x = x;
        this.y = y;
        this.w = w;
        this.h = h;
    ...
    public void render(int mx, int my) {
        if (AutoBuyGui.isHover(x, y - 14, 32, 16, mx, my)) {
            offset = Animation.fast(offset, 8);
            if (alpha < 255) alpha += 15;
        } else {
            if (alpha > 0) alpha -= 15;
            offset = Animation.fast(offset, 0);
        }
      ...
      //  RenderUtil.Render2D.drawRoundedRect((float) (x + 4), (float) y, (float) (w - 8), (float) h, 2, new Color(16, 15, 15, 255).getRGB());
        ...
        if (alpha > 0) {
            IFont.drawCenteredXY(IFont.MONTSERRAT_BOLD, " - ", (float) (x + 12), (float) (y + 8 -14), new Color(255, 0, 0, MathHelper.clamp(alpha, 0, 255)), 8);
        }
        GL11.glPushMatrix();
        GL11.glTranslated(offset, 0, 0);
        IFont.drawCenteredY(IFont.MONTSERRAT_MEDIUM, clampName(), (float) (x + 28), (float) (y + h / 2 - 14), Color.WHITE, 8);
        GL11.glPushMatrix();
        GL11.glTranslated(x + 8, y + h / 2 - 8 -14, 0);
        ...
        if (write) {
            if (dot > 45) dot = 0;
            dot++;
            p = replace(override) + (dot > 30 ? "_" : "");
        }
        IFont.drawCenteredY(IFont.MONTSERRAT_MEDIUM, p, (float) (x + w - 8 - IFont.getWidth(IFont.MONTSERRAT_MEDIUM, p, 8)), (float) (y + h / 2 - 14), Color.WHITE, 8);
    }
    ...
    public void click(int mx, int my, int b) {
        if (AutoBuyGui.isHover(x, y - 14, 32, 16, mx, my) && b == GLFW.GLFW_MOUSE_BUTTON_1) {
            Main.abGui.autoBuyButtons.remove(this);
            AutoBuyManager.removeItem(autoBuyItem);
        }

        String p = replace(autoBuyItem.price + "") + "$";
        if (AutoBuyGui.isHover(x + w - 8 - IFont.getWidth(IFont.MONTSERRAT_MEDIUM, p, 8), y - 14, IFont.getWidth(IFont.MONTSERRAT_MEDIUM, p, 8), h, mx, my) && b == GLFW.GLFW_MOUSE_BUTTON_1) {
